...
// A sheet written from a row source, held to the file a built sheet produces from the same rows.
//
// The claim §65 makes is not that the streaming path writes a valid file - that is the floor - but that
// it writes the *same* file the builder does, so that turning it on is not a change to what a reader
// sees. Every test here that says "equivalent" reads both files back through XlsxReader and compares
// the workbooks, which is the only comparison that survives the two paths ordering their parts
// differently inside the zip.
public class StreamedSheetTests
{
    ...
    // The same sheet, built. This is what "defaults reproduce today's file" is measured against.
    ...
    public async Task Writes_the_header_and_every_row()
    {
        ...
        for (var row = 0; row < people.Length; row++)
        {
            ...
            // A date comes back as its serial under a date format, which is what the built path's own
            // file does: the equivalence test below is what holds the two together.
    ...
    // The claim the section stands on at the file level: same rows in, same workbook out.
    [Fact]
    ...
    // Bounded by the sample and by nothing else: a row past it cannot widen the column, which is the
    // property that makes sampling a streaming mode rather than a buffer with a nicer name.
    [Fact]
    ...
    // The cancellation the loop's own check cannot see: the token trips and the source then ends, so
    // MoveNextAsync answers false and the write walks out of the loop rather than throwing out of it.
    [Fact]
    ...
    /// <summary>
    /// A table's column names come from the header cells, and a heading that reads as a number has to
    /// survive being written into one.
    /// </summary>
    /// <remarks>
    /// The file was always right - the header is written as a string either way - and it was the table
    /// metadata that disagreed with it, which is what a reader sees in the filter buttons. Asserted on
    /// the written XML rather than on a round trip, because <c>XlsxReader</c> infers a shared string
    /// that looks like a number back into one, which is its own behaviour and not this.
    /// </remarks>
    [Fact]
