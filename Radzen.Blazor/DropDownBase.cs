...
namespace Radzen
{
    ...
    public class DropDownBase<T> : DataBoundFormComponent<T>
    {
        ...
        // Set view of the current multiselect values so IsItemSelectedByValue is O(1) per item rather than
        // an O(selected) scan; rebuilt only when internalValue is reassigned or its element count changes.
        ...
        internal bool IsItemSelectedByValue(object v)
        {
            switch (internalValue)
            {
                ...
                case IEnumerable enumerable:
                    // Rebuilt when the collection reference changes and, via OnParametersSet, once per render
                    // cycle - so an in-place change to the same collection is reflected on the next render.
