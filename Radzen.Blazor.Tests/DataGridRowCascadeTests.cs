...
{
    // RadzenDataGridRow only wraps itself in the EditContext / IRadzenForm cascades when the grid has
    // actually put that row into edit mode. These pin the behaviour that depends on those cascades, so
    // the optimisation cannot silently break inline editing or validation.
    public class DataGridRowCascadeTests
    {
                ...
                p.Add<RenderFragment>(g => g.Columns, builder =>
                {
                    ...
                    builder.AddAttribute(2, nameof(RadzenDataGridColumn<Person>.EditTemplate),
                        (RenderFragment<Person>)(person => eb =>
                        {
                            ...
                            eb.AddAttribute(4, "ValueExpression", (System.Linq.Expressions.Expression<System.Func<string>>)(() => person.Name));
                            eb.CloseComponent();
        ...
        public void EditRow_CascadesEditContextToEditors()
        {
            ...
            // FormComponent asks EditContext for the field's CSS class, so a "valid"/"invalid" class on
            // the editor is direct evidence the row cascaded its EditContext down to it. Without the
            // cascade the editor renders with no validation-state class at all.
        ...
        public void EditRow_CascadesFormSoValidatorsResolveTheirComponent()
        {
            ...
            // UpdateRow only commits when EditContext.Validate() passes. The validator resolves its target
            // through IRadzenForm.FindComponent, which is the row - reached only via the second cascade.
            // Lose that cascade and the validator no-ops, validation passes, and the row leaves edit mode.
            Assert.True(component.Instance.IsRowInEditMode(data[0]),
        ...
        public void ReadOnlyRow_WithInputInPlainTemplate_StillRenders()
        {
            ...
            // Radzen's own demos put a RadzenCheckBox in a plain Template; those rows are never in edit
            // mode, so they render without the cascades. They must still render and bind.
