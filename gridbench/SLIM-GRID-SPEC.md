...
Neither text writer wrote `xml:space="preserve"`, and that was so on master for shared strings. Our
own reader loads a sheet with `XDocument.Load`, which drops text that is only whitespace unless the
attribute is in scope, so a streamed `"   "` read back empty. Inline text now carries the attribute
when its first or last character is XML whitespace, and only then. Shared strings are left as master
has them: fixing them would change the bytes of a save with no source, so that belongs in its own
upstream PR. Four tests hold it, one on the `StreamedCell` path, and six mutations of the condition
are killed, including `char.IsWhiteSpace` for XML's four characters.

The prologue that opens a cell (`<c>`, `r`, and `s` when there is a style) was written out in six
writers, four of them added by this branch. It is now one `WriteCellStart`, folded into the commits
that brought each copy. The gate read 16/0 after it, and 1 differing when the helper was broken, so it
sees this code.
