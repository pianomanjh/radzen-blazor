...
public abstract class StreamedSheet
{
    ...
    /// <summary>
    /// Whether the column titles are written as the first row. The header is frozen and counted in the
    /// table range when it is written.
    /// </summary>
    ...
    /// last row, so the table part costs nothing to size.
    /// </summary>
    ...
    /// element is omitted, which readers are required to tolerate; <see cref="XlsxReader"/> then sizes
    /// the sheet from the rows it finds, with a floor of 100. A sampled write that buffered the whole
    /// source knows the count without being told and writes the element anyway.
    /// </remarks>
    ...
    // One array, refilled and handed out again: a row that outlived the cell it was written to would be
    // the one allocation this whole path exists to remove.
